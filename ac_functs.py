...
# import contextlib
import arithmeticcoding2 as ac2
import  arithmeticcoding_clean as arithmeticcoding
...
class ac_model2:
     ...
     def __init__(self,nsyms,bsfile,ENC):
         ...
         if ENC:
             ...
             bitout = ac2.BitOutputStream(self.ofp)
             self.enc = ac2.ArithmeticEncoder(32, bitout)
         else:
             self.ofp = open(self.bsfile, "rb")
             bitin = ac2.BitInputStream(self.ofp)
             # bitin = contextlib.closing(arithmeticcoding.BitOutputStream(self.ofp))              
             self.dec = ac2.ArithmeticDecoder(32, bitin)
