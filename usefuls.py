...
def ints2bs_fast(ints,nintbit):
    
    inbits = ''
    for inte in ints:
        # intbin = dec2bin(inte)
        # inbits=inbits+dec2bin3(inte,nintbit)
        # i=0
        k=0
        while inte > 0:
            inbits=inbits + str(inte  % 2)
            inte  //= 2
            k+=1
            # i+=1
        inbits = inbits+(nintbit-k)*'0'
    return inbits
...
def write_ints_fast(ints,nintbit,fpath):


    inbits = ints2bs_fast(ints,nintbit)
    # inbits = ints2bs(ints,nintbits)
    inbits = inbits+'1'
...
def dec2bin3(num,nbits):
    ...
    result = (nbits-len(result))*'0'+result
    return result

def dec2bin4(num,nbits):

    resint = np.zeros((nbits,),int)
    # if num == 0:
    #     result = '0'
...
def show_time_spent(time_spent):
    ...
    statement = 'time spent: ' + str(nmins) + 'm ' + str(nsecs) + 's'
    print(statement)
    return statement

def show_est_time(time_spent):
    time_spent = int(time_spent)
    nmins1 = time_spent//60
    nhours = nmins1//60
    nmins = nmins1-nhours*60
    nsecs = time_spent-nhours*3600-nmins*60
    statement = 'est. time left: ' + str(nhours) + 'hrs ' + str(nmins) + ' m ' + str(nsecs) + 's'
    print(statement)
    return statement
