...
def slice_to_8(Loc):
    ...
    Locs = []
    ori_level = int(np.ceil(np.log2(np.max(Loc))))
    ...
    xyzinds = Loc<newmax     #0,0,0
    slinds = xyzinds[:,0]*xyzinds[:,1]*xyzinds[:,2]
    Locs.append( Loc[slinds,:])

    xinds = Loc[:,0]<newmax  #0,0,1
    yinds = Loc[:,1]<newmax 
    zinds = Loc[:,2]>=newmax 
    slinds = xinds*yinds*zinds
    the_diff = np.zeros((np.sum(slinds),3),dtype=Loc.dtype)
    the_diff[:,2] = newmax
    Locs.append( Loc[slinds,:]-the_diff)

    xinds = Loc[:,0]<newmax  #0,1,0
    yinds = Loc[:,1]>=newmax 
    zinds = Loc[:,2]<newmax 
    slinds = xinds*yinds*zinds
    the_diff = np.zeros((np.sum(slinds),3),dtype=Loc.dtype)
    ...
    xinds = Loc[:,0]>=newmax #1,1,1
    yinds = Loc[:,1]>=newmax 
    zinds = Loc[:,2]>=newmax 
    slinds = xinds*yinds*zinds
    the_diff = newmax*np.ones((np.sum(slinds),3),dtype=Loc.dtype)
...
def pcwrite(Loc,ply_path):
    ...
    pcd.points = open3d.pybind.utility.Vector3dVector(Loc)
...
def pcshow_pyto(Location):
    pcd = open3d.geometry.PointCloud()
    pcd.points = open3d.pybind.utility.Vector3dVector(Location)
    open3d.visualization.draw_geometries([pcd]) 
